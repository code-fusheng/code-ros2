...
class TestLogger : public rclcpp :: Node
{
    public:
        TestLogger(std::string name) : Node(name)
        {
            RCLCPP_INFO(this -> get_logger(), "%s Node Is Started.", name.c_str());
            ...
            timer_ = this -> create_wall_timer(
                500ms, std::bind(&TestLogger::timer_callback, this));
        }
    private:
        int log_level;
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<TestLogger>("test_logger");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}